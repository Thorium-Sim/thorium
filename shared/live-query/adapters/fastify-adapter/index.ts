...
export async function liveQueryPlugin<TRouter extends AnyRouter, TContext>(
  ...
  if (createWSContext) {
    ...
    const clients: Map<string | number, ServerClient<TRouter>> = new Map();
    fastify.get("/ws", {websocket: true}, async (connection, req) => {
      try {
        ...
        if (!client) {
          client = new ServerClient(context.id, router, pubsub);
          clients.set(context.id, client);
        }
        await client.initWebSocket(connection, context);
      } catch (err) {
        console.error(err);
      }
    });
...
class ServerClient<TRouter extends AnyRouter> {
  connected = false;
  connection!: SocketStream;
  private context!: inferRouterContext<TRouter>;
  ...
  send(data: SocketMessages) {
    try {
      this.connection.socket.send(this.encode(data));
    } catch (err) {
      console.error(err);
      console.error(data);
    }
              ...
              if (!this.subscriptions.get(id)) {
                ...
                const handleRequest = async (publish: any) => {
                  try {
                    ...
                    this.send({type: "netRequestData", data: {id, data}});
