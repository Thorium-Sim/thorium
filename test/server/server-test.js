...
describe('Server', function() {
  it('should provide a /schema endpoint', function(done) {
    chai.request(server).get('/schema').end(function(err, res) {
      expect(res).to.have.status(200);
      done();
    });
  });
  it('should provide a /status endpoint', function(done) {
    chai.request(server).get('/status').end(function(err, res) {
      expect(res).to.have.status(200);
      done();
    });
  });
  it('should provide a /graphql endpoint', function(done) {
    chai.request(server).get('/graphql').end(function(err, res) {
  ...
  it('should allow posts on /graphql', function(done) {
    chai.request(server).post('/graphql').end(function(err, res) {
      expect(res).to.have.status(400);
