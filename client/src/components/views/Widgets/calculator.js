...
export default class Calculator extends React.Component {
  ...
  _displayNum(num) {
    if (this.state.displayVal.length > 19) {
      // Where var too long for display
      return;
    } else if (
      this.state.displayVal === "0" ||
      this.state.displayVal === "Undefined! 😫"
    ) {
      // Where var is 0, or where result is undefined
      this.setState({
        displayVal: num
      });
    } else if (
      this.state.displayVal.charAt(0) === "-" &&
      this.state.displayVal.charAt(1) === "0"
    ) {
  ...
  render() {
    ...
    return (
      <div className="calculator">
        ...
        />
        <Numbers numbers={numbers} _displayNum={this._displayNum.bind(this)} />
        <Operations
...
class Display extends React.Component {
  render() {
    return <div className="calc-display">{this.props.displayVal}</div>;
  }
}
