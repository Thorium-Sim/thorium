...
const updateCoolant = () => {
  ...
  .forEach(s => {
    ...
    // Trigger Events
    const sysCoolant = Math.min(1, Math.max(0, transferSystem.coolant + 0.004 * sign));
    const tankCoolant = Math.min(1, Math.max(0, s.coolant - 0.004 * sign * s.coolantRate));
    if (sysCoolant === 0 || tankCoolant === 0 || sysCoolant === 1 || tankCoolant === 1) s.transfer = null;
    App.handleEvent({systemId: sysId, coolant: sysCoolant}, "setCoolant");
    App.handleEvent({systemId: s.id, coolant: tankCoolant}, "setCoolant");
    pubsub.publish('coolantSystemUpdate', App.systems.filter(s => (s.coolant || s.coolant === 0) && s.type !== 'Coolant')
    .map(s => {
      return {
        systemId: s.id,
