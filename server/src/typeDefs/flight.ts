  ...
  Mutation: {
    ...
    resetFlight(rootQuery, {flightId, simulatorId, full}) {
      ...
      flight.simulators.forEach((simId: string) => {
        ...
        const cacheIdentifiers: CacheClearIdentifier[] = App.clients
          .filter(c => c.flightId === flightId)
          .map(client => ({id: client.id, connected: client.connected}))
          .concat(
            App.flights
              .filter(f => f.id === flightId)
              .map(flight => ({id: flight.id, connected: true})),
          );
