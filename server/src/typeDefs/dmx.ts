...
const resolver = {
  ...
  Mutation: {
    ...
    dmxSetDuplicate(rootValue, {id, name}) {
      const dmxSet = App.dmxSets.find(f => f.id === id);
      if (!dmxSet) throw new Error("DMX Set not found");
      const {id: setId, ...set} = dmxSet;
      // Duplicate all of the fixtures in the set
      const newFixtures = set.fixtureIds.map(fId => {
        const dmxFixture = App.dmxFixtures.find(f => fId === f.id);
        if (!dmxFixture) throw new Error("DMX Fixture not found");
        const {id: _, ...f} = dmxFixture;
    ...
    dmxConfigDuplicate(rootValue, {id, name}) {
      const dmxConfig = App.dmxConfigs.find(f => f.id === id);
      if (!dmxConfig) throw new Error("DMX Config not found");
      const {id: configId, ...configuration} = dmxConfig;
