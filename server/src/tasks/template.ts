import {Simulator, System, Task} from "@server/classes";
import App from "../app";
...
export default [
  {
    ...
    instructions({
      ...
      simulator: Simulator;
      requiredValues: {preamble: string; system: System};
      task: Task | {};
    }) {
      ...
      if (station && "station" in task && task.station === station.name)
        return reportReplace(`${preamble} Perform the template action.`, {
          simulator,
          system,
        });
