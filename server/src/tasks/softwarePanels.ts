...
import {PanelComponent} from "@server/classes/softwarePanels";
...
export default [
  {
    ...
    instructions({
      ...
      const messages = comps
        ...
        .map(c => {
          return randomFromList(allowedComponents[c.component] as string[])
            ...
            .replace("#INT", Math.round(Math.random() * 7 + 2).toString())
            .replace(
              "#CABLE2",
              randomFromList(
                panelObj?.components
      ...
      return reportReplace(
        `${preamble} Go to the ${panelObj?.name} panel and perform the following operations:
