...
import Probes, {Probe} from "@server/classes/probes";
...
export function getProbeConfig(probes: Probes, probe: Probe) {
  ...
  const probeEquip = probe.equipment.reduce(
    (prev: Record<string, number>, p) => ({...prev, [p.id]: p.count}),
    {},
  );
  // Get the requirements
  const reqEquip = probes.scienceTypes.reduce(
    (prev: Record<string, Record<string, number>>, t) => ({
      ...prev,
