import {Simulator, System} from "@server/classes";
import App from "../app";
import {
  randomFromList,
  partsList,
...
export default function reportReplace(
  report = "",
  {system, simulator}: {system?: System; simulator?: Simulator},
) {
  ...
  if (simulator) {
    ...
    if (crew.length > 0) {
      ...
      const crewText = `${crewmember.rank} ${crewmember.firstName} ${crewmember.lastName}`;
