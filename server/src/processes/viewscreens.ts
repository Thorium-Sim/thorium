...
import {Simulator} from "@server/classes";

type Priority = {priority: number};
const viewscreenCache: {
  id: string;
  RedAlert?: Priority;
  ForwardScans?: Priority;
  Overheating?: Priority;
  ShieldMonitoring?: Priority;
  StealthMonitoring?: Priority;
  CourseCalculation?: Priority;
...
function viewscreenMachine() {
  ...
  App.viewscreens.forEach(viewscreen => {
    ...
    cache.ShieldMonitoring = cache.ShieldMonitoring || {priority: 0};
    ...
    shields.forEach(shield => {
      if (!cache.ShieldMonitoring) return;
      if (
    ...
    const stealth = App.systems.filter(
      s =>
        s.simulatorId === viewscreen.simulatorId && s.type === "StealthField",
    );
    cache.StealthMonitoring = cache.StealthMonitoring || {priority: 0};

    stealth.forEach(s => {
      if (!cache.StealthMonitoring) return;
      if (
    ...
    cache.CourseCalculation = cache.CourseCalculation || {priority: 0};
    // Loop through nav
    nav.forEach(n => {
      if (!cache.CourseCalculation) return;
    ...
    cache.Communications = cache.Communications || {priority: 0};
    comm.forEach(c => {
      if (!cache.Communications) return;
    ...
    // Stars
    cache.Stars = cache.Stars || {priority: 0};
    cache.Stars = {
      priority:
    ...
    const component = Object.keys(cache).reduce((prev: string | null, next) => {
