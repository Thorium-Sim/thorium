...
import {Crew} from "@server/classes";
import App from "../app";
  ...
  App.flights
    ...
    .forEach(f => {
      f.simulators.forEach(s => {
        ...
        const noEscapeDecks = decks
          .filter(c => c.doors === true)
          .map(c => c.id);

        // Start with moving team members
        const teamCrewNeedingMoving = teams.reduce(
          (prev: {deck: string; officer: Crew}[], next) => {
            const location =
