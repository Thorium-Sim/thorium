...
export default class TaskReport {
  ...
  assignTask(stepId, station) {
    ...
    if (!definition || !task) throw new Error("Task definitions not found");
    let assignStation =
      station ||
      ("stations" in definition &&
        randomFromList(definition.stations({simulator}))) ||
      [];
  ...
  static generateReport({
    ...
    const taskTemplates = simulator.damageTasks
      ...
      .filter(
        ({definition}) =>
          definition && definition.active({simulator, requiredValues: {}}),
      );

    const requiredTemplates = taskTemplates.filter(s => s.required);
    const optionalTemplates = taskTemplates.filter(s => !s.required);
    let definitionTemplates = optionalTemplates.reduce(
      (prev: Record<string, typeof optionalTemplates>, next) => {
        if (!next.definition) return prev;
    ...
    const rooms = system
      ? App.rooms
          ...
          .map(r => r.id)
      : App.rooms.filter(r => r.simulatorId === simulator.id).map(r => r.id);
    ...
    // @ts-expect-error Don't feel like digging into the types on this one
    ...
    // @ts-expect-error Don't feel like dealing with it
    tasks = tasks.concat(requiredTemplates.filter(s => s.end).map(createTask));
    ...
    const endMacros: {event: string; args: Record<string, any>}[] = [];
    if (type === "default") {
      endMacros.push({
        event: "repairSystem",
