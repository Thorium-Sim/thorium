...
export class System {
  ...
  generateDamageReport(stepCount = 5) {
    ...
    if (!sim) throw new Error("No simulator found");
        ...
        if (
          ...
          if (optionalSteps[stepIndex].name !== "damageTeam") {
            ...
            optionalSteps = optionalSteps.filter(
              (_: DamageStep, i: number) => i !== stepIndex,
            );
          }
        } else if (
...
`;
  ...
  updateDamageTask(task: DamageTask) {
    const damage = this.damageTasks.find(t => t.id === task.id);
    if (!damage) throw new Error("Damage task not found");
    damage.update(task);
  }
