...
export default class Task {
  ...
  constructor(params: Partial<Task> = {}) {
    ...
    if (!definitionObject) throw new Error("Unable to find task definition");
    ...
    const stations =
      "stations" in definitionObject
        ? (definitionObject.stations({simulator}) as Station[])
        : (simulator?.stations as Station[]);
