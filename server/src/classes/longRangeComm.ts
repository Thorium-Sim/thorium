...
class LRMessage {
  ...
  deleted: boolean;
  sent: boolean;
  constructor(params: Partial<LRMessage> = {}, simulatorId) {
    ...
    let station = false;
    if (simulator) {
      station = simulator.stations.some(s =>
...
export default class LongRangeComm extends System {
  constructor(params: Partial<LongRangeComm> = {}) {
    super({
  ...
  createMessage(message, crew, decoded, sender) {
    const params: {
      ...
      ra?: number;
      rf?: number;
    } = {message, crew, sender, sent: false};
    if (decoded) {
