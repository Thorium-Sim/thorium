...
export default class Sensors extends System {
  ...
  constructor(
    params: Partial<Sensors> & {processedData: ProcessedData[] | string},
    newlyCreated = false,
  ) {
    super(params);
    this.class = "Sensors";
  ...
  updateScan(scan) {
    const scans = this.scans.find(s => s.id === scan.id);
    if (!scans) throw new Error("Scan not found");
    scans.update(scan);
  }
  cancelScan(id) {
    const scans = this.scans.find(s => s.id === id);
    if (!scans) throw new Error("Scan not found");
    scans.update({response: "", cancelled: true});
  }
