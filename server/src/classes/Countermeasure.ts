...
class CountermeasureModule {
  ...
  constructor({
    ...
    buildProgress = 0,
    activated = false,
  }: Omit<CountermeasureModule, "activate">) {
...
export class Countermeasures extends System {
  ...
  constructor(
    params: Partial<Omit<Countermeasures, "slots">> & {
      slots: CountermeasureSlots;
    },
  ) {
  ...
  get materials() {
    const usedMaterials = Object.entries(this.slots).reduce(
      (prev, [, slot]) => {
        slot?.modules.forEach(mod => {
          prev.copper +=
            (mod.resourceRequirements.copper || 0) * (mod.buildProgress || 0);
          prev.carbon +=
            (mod.resourceRequirements.carbon || 0) * (mod.buildProgress || 0);
          prev.plasma +=
            (mod.resourceRequirements.plasma || 0) * (mod.buildProgress || 0);
          prev.plastic +=
            (mod.resourceRequirements.plastic || 0) * (mod.buildProgress || 0);
          prev.titanium +=
            (mod.resourceRequirements.titanium || 0) * (mod.buildProgress || 0);
        });
        return prev;
      },
    ...
    return {
      copper: this.storedMaterials.copper
        ? this.storedMaterials.copper - usedMaterials.copper
        : 0,
      carbon: this.storedMaterials.carbon
        ? this.storedMaterials.carbon - usedMaterials.carbon
        : 0,
      plasma: this.storedMaterials.plasma
        ? this.storedMaterials.plasma - usedMaterials.plasma
        : 0,
      plastic: this.storedMaterials.plastic
        ? this.storedMaterials.plastic - usedMaterials.plastic
        : 0,
      titanium: this.storedMaterials.titanium
        ? this.storedMaterials.titanium - usedMaterials.titanium
        : 0,
    };
  }
  ...
  removeCountermeasure(slot) {
    if (this.slots[slot]) {
      const countermeasure: Countermeasure | null = this.slots[slot];
      ...
      countermeasure.modules.forEach(m => {
        ...
        const usedMaterialValue = (m.buildProgress || 0) * 0.5;
  ...
  launchCountermeasure(slot) {
    const slotItem = this.slots[slot];
    if (!slotItem) return;
    if (slotItem.readyToLaunch) {
      ...
      slotItem.modules.forEach(m => m.activate());

      this.launched.push(slotItem);
      ...
      countermeasure?.modules.forEach(m => {
        ...
        Object.entries(m.resourceRequirements).forEach(([key, value]) => {
          this.storedMaterials[key] = Math.round(
            this.storedMaterials[key] - (usedMaterialValue || 0) * value,
          );
        });
      });
    }
  ...
  get readyToLaunch() {
    ...
    return (
      ...
      this.modules.reduce((acc, next) => {
        if (!acc) return false;
        if (!next.buildProgress) return false;
