...
export default class Probes extends System {
  ...
  scienceTypes: typeof scienceProbeTypes;
  constructor(params: Partial<Probes> = {}, init?: boolean) {
    super(params);
    this.class = "Probes";
    this.type = "Probes";
    ...
    params.types =
      (init
        ? probesTypes.map(type => new ProbeType(type, this.id))
        : params.types) ||
      probesTypes.map(type => new ProbeType(type, this.id));
    params.equipment = init
      ? probesEquipment.map(equip => new ProbeEquipment(equip))
      : params.equipment ||
        probesEquipment.map(equip => new ProbeEquipment(equip));
...
export class Probe {
  ...
  response: string;
  charge: number;
  history: {date: Date; text: string}[];
  constructor(params: Partial<Probe>, parentId: string) {
...
class ProbeType {
  ...
  update({name, description, size, count}: Partial<ProbeType>) {
...
class ProbeEquipment {
  ...
  update({name, description, size, count}: Partial<ProbeEquipment>) {
    if (name) this.name = name;
    if (description) this.description = description;
