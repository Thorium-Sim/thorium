...
export class StationSet {
  ...
  static import(data: StationSet) {
    const stationSet = new StationSet({...data, id: undefined});
  ...
  setStationMessageGroup(station: string, group: string, state: boolean) {
    const stationObj = this.stations.find(s => s.name === station)
    if (!stationObj) throw new Error("Station not found")
    stationObj.setMessageGroup(group, state);
  }
  setStationLogin(station: string, login: boolean) {
    const stationObj = this.stations.find(s => s.name === station)
    if (!stationObj) throw new Error("Station not found")
    stationObj.setLogin(login);
  }
  setStationExecutive(station: string, exec: boolean) {
    const stationObj = this.stations.find(s => s.name === station)
    if (!stationObj) throw new Error("Station not found")
    stationObj.setExec(exec);
  }
  setStationWidget(station: string, widget: string, state: boolean) {
    const stationObj = this.stations.find(s => s.name === station)
    if (!stationObj) throw new Error("Station not found")
    stationObj.setWidgets(widget, state);
  }
  setStationLayout(station: string, layout: string) {
    const stationObj = this.stations.find(s => s.name === station)
    if (!stationObj) throw new Error("Station not found")
    stationObj.setLayout(layout);
  }
...
export class Station {
  ...
  reorderWidgets(widget: string, order: number) {
    function move(array: (string|undefined)[], old_index: number, new_index: number) {
      ...
      return array.filter(Boolean) as string[]; // for testing purposes
    }
