...
export default class Simulator {
  ...
  setTimelineStep(step: number, timelineId: string | null = null) {
    if (timelineId) {
  ...
  updateCode(codeId: string, state: "Denied" | "Accepted" | "sent") {
    const code = this.ship.remoteAccessCodes.find(c => c.id === codeId);
    if (code) {
      code.state = state;
    }
  }
  setSelfDestructTime(time: number | null) {
  ...
  hideCard(cardName: string) {
    ...
    const cards = this.stations
      ? this.stations.reduce((acc: Card[], s) => acc.concat(s.cards), [])
      : [];
    cards.forEach(card => {
      if (
        card &&
