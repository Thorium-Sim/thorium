...
import { Container, Row, Col, Card, CardBlock, Button, Label } from 'reactstrap';
...
class Decoding extends Component {
  ...
  decode() {
    ...
    if (decodeProgress < message.length) {
      ...
      // Update the server
      const mutation = gql `mutation UpdateDecodedMessage($id: ID!, $messageId: ID!, $decodedMessage: String, $a: Int, $f: Int) {
        updateLongRangeDecodedMessage(id: $id, messageId: $messageId, decodedMessage: $decodedMessage, a: $a, f: $f)
      }`;
      const variables = {
  ...
  _selectMessage(message) {
    ...
    this.setState({
      ...
      a: message.a,
      f: message.f,
      decodedMessage: (message.a === message.ra && message.f === message.rf) ? message.message : ''
    });
  }
  ...
  _handleChangeComplete(which, e){
    ...
    const mutation = gql `mutation UpdateDecodedMessage($id: ID!, $messageId: ID!, $a: Int, $f: Int) {
      updateLongRangeDecodedMessage(id: $id, messageId: $messageId, a: $a, f: $f)
    }`;
    const variables = {
      id: this.props.data.longRangeCommunications[0].id,
      messageId: selectedMessage,
  ...
  render() {
    ...
    <Button className="decode-button" disabled={!this.state.selectedMessage} color="secondary" onClick={() => {this.setState({decodedMessage:'', message:selectedMessage.message, ra:selectedMessage.ra, rf:selectedMessage.rf}, this.decode.bind(this)); }}>Decode</Button>
    <Row>
    <Col>
    ...
    <Card>
    { sys.messages.map(m => (
      <li key={m.id} onClick={this._selectMessage.bind(this, m)}
...
class DecodingCanvas extends Component {
  ...
  render() {
    ...
    return (
      ...
      <Layer>
      <Line
      points={sinPoints({f, a, message, width: this.props.dimensions.width})}
      stroke='red'
      strokeWidth={2}
      lineJoin="round"
      lineCap="round"/>
      <Line
      points={decodeProgress ? decodePoints({rf, ra, message, decodeProgress, width: this.props.dimensions.width}) : sinPoints({f: rf, a: ra, p: p, animate: true, width: this.props.dimensions.width})}
