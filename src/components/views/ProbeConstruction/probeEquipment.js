...
export default class ProbeEquipment extends Transitioner {
  ...
  render() {
    ...
    return (
      ...
      <CardBlock onMouseOut={()=>{this.setState({shownDescription:null})}} className="equipmentList">
      {
        type.availableEquipment.map(e => {
          const used = equipment.find(eq => eq.id === e.id) || {count: 0};
          return <Row onClick={this.addToProbe.bind(this, e)} onMouseOver={() => {this.setState({shownDescription: e.description})}} className="equipmentItem">
          <Col sm="8">
          <p>{e.name}</p>
          </Col>
          <Col sm="2">
      ...
      <Button block color="primary" onClick={prepareProbe.bind(this, equipment)}>Prepare Probe</Button>
      ...
      {
        equipment.map(e => (
          <Row onClick={this.removeFromProbe.bind(this, e)} onMouseOver={() => {this.setState({shownDescription: e.description})}} className="equipmentItem">
          <Col sm="8">
          <p>{e.name}</p>
          </Col>
          <Col sm="2">
