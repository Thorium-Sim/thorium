...
import { Container, Button, Input, Row, Col, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap';
...
class ProbeConstruction extends Component {
  ...
  render(){
    ...
    <TransitionGroup>
    {      
      [ProbeDescription, ProbeEquipment, ProbeAction].filter(Comp => {
        if (Comp.name === 'ProbeDescription' && !selectedProbeType) return true;
  ...
  const ProbeSelector = ({types, selectedProbeType, selectProbe, setDescription}) => {
    return <Row>
  ...
  class ProbeAction extends Transitioner {
    render() {
      ...
      return (
        ...
        <Col sm={{size: 4, offset: 4}}>
        <Button block color="success" onClick={toggle} >Launch Probe</Button>
        <Button block color="warning" onClick={selectProbe.bind(this, selectedProbeType)}>Reconfigure Probe</Button>
        <Button block color="danger" onClick={cancelProbe}>Cancel Probe</Button>
        </Col>
        </Row>
        );
    }
  ...
  class ProbeName extends Component {
    ...
    render(){
      const {modal, toggle, launchProbe} = this.props;
      const {name} = this.state
      return <Modal className="modal-themed" isOpen={modal} toggle={toggle}>
      <ModalHeader toggle={toggle}>Please enter a name for the probe</ModalHeader>
      <ModalBody>
        <Input type="text" value={name} onChange={(e) => this.setState({name: e.target.value})} />
      </ModalBody>
      <ModalFooter>
      <Col sm="3">
      <Button block color="danger" onClick={toggle}>Cancel</Button>
      </Col>
      <Col sm="5">
      <Button block disabled={!name} color="primary" onClick={launchProbe.bind(this, this.state.name)}>Launch Probe</Button>
      </Col>
      </ModalFooter>
      </Modal>
    }
