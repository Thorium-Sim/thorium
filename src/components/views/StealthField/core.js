...
class StealthFieldCore extends Component {
  ...
  render(){
    ...
    const highSystemsText = highSystems.reduce((prev, s, i) => {
      let returnString = `${this.systemName(s)} (${Math.round(s.stealthFactor * 100)})`;
    ...
    <label> Activate <input checked={stealthField.activated} onChange={this.updateActivated.bind(this)} type="checkbox" /></label>
    <label>Charge <input disabled checked={stealthField.charge} onChange={this.updateCharge.bind(this)} type="checkbox" /></label>
    <Row>
    <Col sm="12">
    <OutputField onMouseDown={this.toggleStealth.bind(this)} alert={stealthField.state}>{stealthField.state ? "Activated" : "Deactivated"}</OutputField>
    </Col>
    <Col sm="12">
    <OutputField id="stealth-systems" style={sysStyle} alert={alertHigh} title={highSystemsText}>
    {
      highSystems.length > 1 ? 
      `${highSystems.length} Alert Systems` : 
      `${highSystems[0].name} (${Math.round(highSystems[0].stealthFactor * 100)})`
    }
    </OutputField>
    </Col>
    </Row>
