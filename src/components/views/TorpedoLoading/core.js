...
class TorpedoLoadingCore extends Component {
  ...
  render(){
    ...
    const types = torpedos.inventory.reduce((prev, next) => {
      if (prev[next.type]){
        prev[next.type] += 1;
      } else {
        prev[next.type] = 1;
    ...
    return (
      ...
      <OutputField alert={torpedos.state === 'fired'}>
      {
        (() => {
          ...
          return `${this.state.type} Torpedo ${torpedos.state === 'loaded' ? 'Loaded' : 'Fired'}`;
        })()
      }
      </OutputField>
      <Row>
      ...
      <Col sm={6}>
      <InputField prompt='What is the new value for photon torpedos?'
      onClick={this.updateTorpedoCount.bind(this, 'photon')}>{types.photon}</InputField>
      </Col>
      <Col sm={6}>
      Quantum: 
      </Col>
      <Col sm={6}>
      <InputField prompt='What is the new value for quantum torpedos?'
      onClick={this.updateTorpedoCount.bind(this, 'quantum')}>{types.quantum}</InputField>
      </Col>
      <Col sm={6}>
      Probe:
      </Col>
      ...
      onClick={this.addTorpedo.bind(this)}>Add Torpedo</Button>
      </Col>
      <Col sm={6} style={{position: 'absolute', right:0, height: '100%', overflow: 'scroll'}}>
      <p>Other</p>
      
      {
        ...
        .map((t) => {
          ...
          <Col sm={6}>
          <InputField prompt={`What is the new value for ${t} torpedos?`}
          onClick={this.updateTorpedoCount.bind(this, t)}>{types[t]}</InputField>
          </Col>
          </Row>
        })
      }
