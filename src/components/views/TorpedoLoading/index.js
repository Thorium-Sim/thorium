    ...
    if (!this.subscription && !nextProps.data.loading) {
      this.subscription = nextProps.data.subscribeToMore({
        ...
        updateQuery: (previousResult, {subscriptionData}) => {
          ...
          return returnResult.merge({torpedos: subscriptionData.data.torpedosUpdate}).toJS();
        }
      });
    }
...
const Torpedo = ({state, type, updateScreen}) => {
  ...
  return <div className="torpedoHolder">
  <svg className="torpedo" style={style} width="100%" height="100%" viewBox="0 0 32 10">
...
class TorpedoTube extends Transitioner {
  render(){
    const {torpedoState, torpedoType, updateScreen, unloadTorpedo, fireTorpedo, enabled} = this.props;
    return <div style={{position: 'absolute', width: '100%', height: '100%'}}>
    <div className="torpedoButton">
    {torpedoState === 'idle' ?
    <Button block color="info" disabled={!enabled} onClick={updateScreen.bind(this, 'TorpedoPick')}>Load Torpedo</Button>
    : <div>
    <Button block color="warning" disabled={!enabled} onClick={unloadTorpedo}>Unload Torpedo</Button>
    <Button block color="danger" disabled={!enabled} onClick={fireTorpedo}>Fire Torpedo</Button>
    </div>
  }
  </div>
...
class TorpedoPick extends Transitioner {
  render() {
    ...
    <div className="torpedoPicker"  style={{width: Object.keys(types).length * torpedoWidth}}>
    {
      Object.keys(types).map((t,i) => {
        ...
        return <div key={t + i} onClick={loadTorpedo.bind(this, (inventory.find(i => i.type === t)||{}).id)} className="torpedoPick" style={{width: torpedoWidth}}>
        <img draggable="false"  role="presentation" src={img} />
        <span style={{textTransform: 'capitalize'}}>{t} ({types[t]})</span>
        </div>
      })
    }
    </div>
