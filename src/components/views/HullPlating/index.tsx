...
const TrainingSteps = [
  {
    ...
    content: <FormattedMessage id={'hull-plating-training-1'} defaultMessage="This system allows you to strengthen your ship's outer hull, reducing damage from difference types of projectiles or energy sources. To activate your armor, press the button here" />
  },
  {
    selector: '.mode-btns',
    content: <FormattedMessage id={'hull-plating-training-2'} defaultMessage="When the system is engaged, you can select the mode you would like to use. Each mode has different advantages. Try different modes to gain difference advantages" />
  },
  {
    selector: ".effective-chart",
    content: <FormattedMessage id={'hull-plating-training-3'} defaultMessage="This graph shows how well the armor is working. If the line is flat, it's not very effective. If the line moves more rapidly, the armor is more effective. If you change armor types, it will take some time for the armor to become effective again." />
  }
]
...
const HullPlatingComp: React.FC<HullPlatingProps> = props => {
  ...
  const generateHullPlatingImg = (hullPlating: HullPlating) => {
    ...
    return <img style={{ height: '90vh', marginTop: '-3vh' }} alt="hull plating background" src={src} />
  ...
  const generateEnergyVideo = (hullPlating: HullPlating) => {
    ...
    if (hullPlating.engaged) {
      if (hullPlating.mode === 'kinetic') {
        src = require(`./videos/Kinetic-${hullPlating.pulse ? 'High' : 'Low'}.mp4`)
      }
      else if (hullPlating.mode === 'energy') {
        src = require(`./videos/Energy-${hullPlating.pulse ? 'High' : 'Low'}.mp4`)
      }
      else {
        ...
        <div className="hull-plating-img-parent">
          ...
          <div className="hull-plating-activate-btn-parent">
            <div className="activate-btn">
              {!hullPlating.engaged && <Button onMouseDown={() => handleEngageClick()} color={"primary"} block={true} size={'lg'}><FormattedMessage id={'hull-plating-engage'} defaultMessage="Engage" /> </Button>}
              {hullPlating.engaged && <Button onMouseDown={() => handleEngageClick()} color={"danger"} block={true} size={'lg'}><FormattedMessage id={'hull-plating-disengage'} defaultMessage="Disengage" /></Button>}
            </div>
          </div>
        </div>
      </Col>
        ...
        <div className="hull-plating-buttons-parent">
          <div className="mode-btns">
            {HullPlatingModeConstants.map((each) => {
              ...
              return <Button color={each.color} onMouseDown={generateClickFunction(each.value)}>{each.name}</Button>
            })}
          </div>
