...
import { Input, Row, Col, FormGroup, Form, Label, Card, CardBlock, Button } from 'reactstrap';
...
const ContactContextMenu = (props) => {
  ...
  return (
    ...
    <Col sm={12}>
        <FormGroup>
    <Label for="iconSelect" sm={2}>Label</Label>
    <Input type="text" name="label" id="labelText" defaultValue={contact.name} onChange={(e) => {
    ...
    <input type="range" id="sizeRange" min="0.1" defaultValue={contact.size} max="5" step="0.1"  onChange={(e) => {
        const contact = props.contact;
    ...
    <FormGroup>
    <Label>
    <Input type="color" id="colorInput" style={{width: '20px', padding: 0}}defaultValue={contact.color} onChange={(e) => {
        const contact = props.contact;
        console.log('Color!',contact);
        props.updateArmyContact(contact, 'color', e.target.value);
    }} />{' '}
    </Label>
    </FormGroup>
    </FormGroup>
