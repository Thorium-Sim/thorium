...
class InternalCommCore extends Component {
  ...
  call(e){
    ...
    const incoming = (room ? `${room.name}, Deck ${deck.number}` : `Deck ${deck.number}`);
    const mutation = gql`
  ...
  render() {
    ...
    return (<div className="internal-comm-core">
      <p>Internal Comm</p>
      {
        (this.props.data.internalComm.length > 0 ? <Container>
          <Row>
          <Col sm={8}>
          <p style={{background: 'lightgray', height: '16px', padding: '0 5px'}}>
          {internalComm.outgoing}{internalComm.state === 'connected' ? ` - Connected` : ''}
          </p>
          </Col>
          <Col sm={4}>
          {
            internalComm.state === 'connected' ? 
            <Button block size="sm" color="danger" onClick={this.cancelCall.bind(this)}>Disconnect</Button> :
            <Button block disabled={!(internalComm.state !== 'connected' && internalComm.outgoing)} size="sm" color="info" onClick={this.connect.bind(this)}>Connect</Button>
          }
          ...
          </Row>
          {
            ...
            <Col sm={12}>
            {
              internalComm.state === 'connected' ?
              <Button block size="sm" color="danger" onClick={this.cancelCall.bind(this)}>Disconnect</Button> :
              <Button block size="sm" color="warning" onClick={this.cancelCall.bind(this)}>Cancel Call</Button>
            }
            </Col>
            </Row> :
            <Row>
            ...
            <Button
            block
            disabled={!deck || deck === 'Select Deck' || internalComm.state === 'connected'}
            size="sm"
            color="success"
