...
class Login extends Component {
  ...
  _login() {
    ...
    if (loginName.length > 0){
      ...
      const mutation = gql`mutation LoginClient ($client: ID!, $loginName: String){
  ...
  render(){
    ...
    return (
      ...
      <h2>Agent Login</h2>
      <Input onKeyDown={(e) => {if (e.which === 13){this._login.call(this)}}} value={this.state.loginName} onChange={this._updateName.bind(this)} ref="loginField" />
      <Button onClick={this._login.bind(this)} block>Login</Button>
      </Col>
      </Col>
      </Row>
      );
