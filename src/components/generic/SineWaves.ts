...
class Utilities {
  ...
  static getFn<T>(obj: Record<string, T>, name: T | string, def: string): T {
    if (Utilities.isFunction(name)) {
      ...
      Utilities.isString(name) &&
      Utilities.isFunction(obj[name as string])
    ) {
...
class SineWaves {
  ...
  private getPoint(time: number, position: number, options: WaveOptions) {
    const x =
      ...
      (-this.yAxis + position) / options.wavelength!;
  ...
  private update(): void {
    ...
    this.ctx.save();
    this.waves.forEach(wave => {
      this.ctx.strokeStyle = wave.strokeStyle || "";
      this.ctx.lineWidth = wave.lineWidth || 1;
      this.drawWave(this.time * (wave.timeModifier || 1), wave);
    });
    this.ctx.restore();
  }
