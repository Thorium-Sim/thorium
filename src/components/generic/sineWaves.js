 ...
 */
export function SineWaves(options) {
  // Save a reference
...
SineWaves.prototype.setupWaveFns = function () {
  ...
  while (++index < length) {
    this.waves[index].waveFn = Utilities.getFn(
      Waves,
      this.waves[index].type,
      "sine",
    );
  }
};
...
SineWaves.prototype.getPoint = function (time, position, options) {
  var x =
    time * this.options.speed + (-this.yAxis + position) / options.wavelength;
  var y = options.waveFn.call(this, x, Waves);

  // Left and Right Sine Easing
  var amplitude = this.easeFn.call(
    this,
    position / this.waveWidth,
    options.amplitude,
  );
