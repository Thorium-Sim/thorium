...
import { Container, Row, Col, Card, ButtonGroup, Button, Input } from 'reactstrap';
...
const ops = {
  create:gql`mutation CreateStationSet($id: ID!, $name: String!) {
    createStationSet(name: $name, simulatorId: $id)
  }`,
  rename:gql`mutation RenameStationSet($id: ID!, $name: String!) {
    renameStationSet(stationSetID: $id, name: $name)
  }`,
  remove:gql`mutation RemoveStationSet($id: ID!) {
    removeStationSet(stationSetID: $id)
  }`,
  renameStation: gql`mutation RenameStation($id: ID!, $name: String!, $newName: String!){
  ...
  updateStationCard: gql`mutation EditCard($stationSetId: ID!, $stationName: String!, $cardName: String!, $name: String, $component: String, $icon: String){
...
class SimulatorConfigView extends Component {
  ...
  renameStationSet = () => {
    let name  = prompt('What is the new station set name? eg. 12-Standard, 8-School, etc.');
    if (name) {
      const variables = {
  ...
  render(){
    ...
    return(
      ...
      <Col sm={3}>
      <Card>
      {stationSets.map(s => <li key={s.id}
        className={`list-group-item ${selectedStationSet === s.id ? 'selected' : ''}`}
        onClick={() => this.setState({selectedStationSet: s.id})}>
        {s.name}
        </li>)}
      </Card>
      <ButtonGroup>
      <Button onClick={this.createStationSet} size="sm" color="success">Add</Button>
      <Button onClick={this.showImportModal} size="sm" color="info">Import</Button>
      </ButtonGroup>
      <ButtonGroup>
      {selectedStationSet && <Button onClick={this.renameStationSet} size="sm" color="warning">Rename</Button>}
      {selectedStationSet && <Button onClick={this.removeStationSet} size="sm" color="danger">Remove</Button>}
      </ButtonGroup>
      </Col>
      <Col sm={9}>
      {selectedStationSet && <ConfigStationSet client={this.props.client} selectedStationSet={stationSets.find(s => s.id === selectedStationSet)} /> }
      </Col>
      </Row>
      </Container>
      );
...
const ConfigStationSet = ({client, selectedStationSet}) => {
  ...
  {selectedStationSet.stations.map((station,stationIndex) => {
    return (
      <div key={`${selectedStationSet.id}-${station.name}-${stationIndex}`} style={{marginBottom: '15px', border: 'solid 1px rgba(0,0,0,0.5)'}}>
      <table className="table table-sm table-striped table-hover">
      <thead className="thead-default">
      <tr>
      <th colSpan="3">{station.name}</th>
      <th><FontAwesome name="pencil-square-o" className="text-warning" onClick={() => renameStation(station)} /> 
      <FontAwesome name="ban" className="text-danger" onClick={() => removeStation(station)} /></th>
      </tr>
      </thead>
      <thead>
      <tr>
      ...
      {station.cards.map((card, index) => {
        return (
          ...
          <td><Input type="text" value={card.name} onChange={(e) => updateStationCard('name', card, station, e)} /></td>
          <td>
          <Input type="select" value={card.component} onChange={(e) => updateStationCard('component', card, station, e)}>
          {
            viewList.map((e) => {
          ...
          <td><FontAwesome name="ban" className="text-danger" onClick={() => removeCard(card,station)} /></td>
          </tr>
          );
      })}
      </tbody>
      ...
      <select className="c-select form-control" onChange={(e) => addCard(station, e)}>
  ...
  <Button size="sm" color="success" onClick={addStation}>Add Station</Button>
  </div>
}
