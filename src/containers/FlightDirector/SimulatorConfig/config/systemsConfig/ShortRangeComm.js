...
const ops = {
  ...
  removeSignal: gql`mutation RemoveCommSignal($id: ID!, $signalId: ID!) {
    commRemoveSignal(id: $id, signalId: $signalId)
  }`,
  updateSignal: gql`mutation CommSignalUpdate($id: ID!, $signal: CommSignalInput!){
...
const ShortRangeComm = ({data, client, simulatorId, type}) => {
  const defaultSignals = (which, {id, signals}) => {
    ...
    const defaultSignals = {
      trek: [ {"image":"Romulan","name":"Romulan","color":"#00ff00","range":{"upper":1,"lower":0.85}},{"image":"Cardassian","name":"Cardassian","color":"#ffaa00","range":{"upper":0.85,"lower":0.75}},{"image":"General Use","name":"General Use","color":"#ffffff","range":{"upper":0.75,"lower":0.58}},{"image":"Starfleet","name":"Starfleet","color":"#0088ff","range":{"upper":0.58,"lower":0.4}},{"image":"Klingon","name":"Klingon","color":"#ff0000","range":{"upper":0.4,"lower":0.3}},{"image":"Orion","name":"Orion","color":"#888888","range":{"upper":0.3,"lower":0.22}},{"image":"Ferengi","name":"Ferengi","color":"#ffff00","range":{"upper":0.22,"lower":0}}]
    }
  ...
  {shortRangeComm.map(e => <div key={e.id}>
    ...
    {e.signals.map(s => <div key={s.id} style={{border: 'solid 1px rgba(0,0,0,0.25)'}}>
      <FormGroup>
      <Label style={{display: 'inline-block', marginRight: '5px'}}>Name
      <Input type="text" value={s.name} onChange={(evt) => {signalUpdate('name', s, e, evt.target.value)}} />
      </Label>
      <Label style={{display: 'inline-block', marginRight: '5px'}}>Color
      <Input style={{height: '30px', paddingTop: '0px', paddingBottom: '0px'}}type="color" value={s.color} onChange={(evt) => {signalUpdate('color', s, e, evt.target.value)}} />
      </Label>
      <Label style={{display: 'inline-block', marginRight: '5px'}}>Image
      <Input type="select" value={s.image} onChange={(evt) => signalUpdate('image', s, e, evt.target.value)}>
      {containers.map(i => <option key={i.id} value={i.name}>{i.name}</option>)}
      </Input>
      </Label>
      <Button size="sm" color="danger" onClick={() => removeSignal(e.id, s.id)}>Remove Signal</Button>
      </FormGroup>
      <FormGroup>
      <Label style={{display: 'inline-block', marginRight: '5px'}}>Range Lower
      <Input type="range" min="0" max="100" step="1" value={s.range.lower*100} onChange={(evt) => {signalUpdate('range-lower', s, e, evt.target.value/100)}} />
      {s.range.lower}
      </Label>
      <Label style={{display: 'inline-block', marginRight: '5px'}}>Range Upper
      <Input type="range" min="0" max="100" step="1" value={s.range.upper*100} onChange={(evt) => {signalUpdate('range-upper', s, e, evt.target.value/100)}} />
      {s.range.upper}
      </Label>
      </FormGroup>
      </div>)
