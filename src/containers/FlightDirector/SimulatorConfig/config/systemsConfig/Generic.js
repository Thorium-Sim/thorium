...
const ops = {
  updatePowerLevels: gql`mutation UpdatePowerLevels($id: ID!, $levels: [Int]!) {
...
export class GenericSystemConfig extends Component {
  ...
  render() {
    ...
    {systems.length === 0 && <p>Click the checkbox to add a {this.props.type} system</p>}
    {systems.map(s => <div key={s.id} style={{border: 'solid 1px rgba(0,0,0,0.5)', padding: '2px'}}>
      <label>
      {s.type}
      </label>
      <FormGroup>
      <Label>Name
      <Input type="text" value={s.name} onChange={(e) => {this.updateName(s,e.target.value,null)}} />
      </Label>
      </FormGroup>
      <FormGroup>
      <Label>Display Name
      <Input type="text" value={s.displayName} onChange={(e) => {this.updateName(s,null,e.target.value)}} />
      </Label>
      </FormGroup>
      <FormGroup>
      <Label>Required Power</Label>
      {s.power.powerLevels && s.power.powerLevels.map((p, i) => <div key={`system-power-${i}`} style={{display: 'inline-block'}}>
        <Input style={{width: '40px', padding: '2px', display: 'inline-block'}} 
        type="number" 
        name="power" 
        value={p}
        onChange={(e) => this.updatePowerLevel(s, i, e.target.value)} />
        </div>)}
      <FontAwesome onClick={() => {this.addPowerLevel(s)}} name="plus-circle" className="text-success" />
      <FontAwesome onClick={() => {this.removePowerLevel(s)}} name="minus-circle" className="text-danger" />
      </FormGroup>
          {this.props.children}
