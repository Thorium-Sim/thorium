...
const ops = {
  addSystem: gql`mutation AddSystemToSimulator($id: ID!, $params: String = "{}") {
    addSystemToSimulator(simulatorId: $id, className: "Reactor", params: $params)
  }`,
  removeSystem: gql`mutation RemoveSystem($id: ID) {
    removeSystemFromSimulator(systemId: $id)
  }`,
  updateOutput: gql`mutation UpdateReactorOutput($id: ID!, $output: Int!) {
...
const Reactor = ({data, client, simulatorId, type}) => {
  ...
  const {reactors} = data;
  return <div className="reactor scroll">
  {reactors.map(e => <div key={e.id}>
    ...
    <p>Model: {e.model}</p>
    {
      ...
      <Input type="number" value={e.powerOutput} onChange={(evt) => {updateOutput(e, evt.target.value)}} />
      </Label>
      </FormGroup>
    }
    <Button color="danger" onClick={() => {removeReactor(e.id)}}>Remove Reactor</Button>
    </div>
    </GenericSystemConfig>
    </div>)}
  <Input color="primary" type="select" value="label" onChange={(e) => addReactor(e.target.value)}>
  <option value="label" disabled>Add Reactor</option>
