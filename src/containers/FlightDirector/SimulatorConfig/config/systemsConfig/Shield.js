...
const ops = {
  addSystem: gql`mutation AddSystemToSimulator($id: ID!, $params: String!) {
    addSystemToSimulator(simulatorId: $id, className: "Shield", params: $params)
  }`,
...
const Shields = ({data, client, simulatorId, type}) => {
  ...
  {shields.map(e => <GenericSystemConfig key={e.id} client={client} simulatorId={simulatorId} type={type} data={{systems: [e]}} />)}
  </div>
}
