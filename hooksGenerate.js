/* This is slightly modified code from https://unpkg.com/@graphql-codegen/typescript-react-apollo@1.12.0/index.esm.js */
 ...
 */`;
  ...
  _buildHooks(
    ...
    if (operationType === "Query") {
      ...
      hookFns.push(`export function use${operationName}Sub(baseOptions?: ApolloReactHooks.${operationType}HookOptions<${operationResultType}, ${operationVariablesTypes}>) {
          return use${operationType}Sub(${this.getDocumentNodeVariable(
        node,
        documentVariableName,
      )}, ${this.getDocumentNodeVariable(node, documentVariableName).replace(
        "Document",
        "UpdateDocument",
      )}, baseOptions);
        }`);
  ...
  buildOperation(
    ...
    const mutationOptionsType = this.config.withMutationOptionsType
      ...
      : null;
    return [mutationFn, component, hoc, hooks, resultType, mutationOptionsType]
      .filter(a => a)
      .join("\n");
  }
}
