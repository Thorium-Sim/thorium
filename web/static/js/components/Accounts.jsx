...
import { Col, Row, Container, Card, CardBlock, CardTitle, Button, ButtonGroup } from 'reactstrap';
...
export class SigninView extends Component {
	...
	_handleEmailChange(event) {
		...
		if (atpos < 1 || dotpos < atpos + 2 || dotpos + 2 >= x.length) {
			...
			this.setState({
				emailInvalid: "Email address required",
				email: x
			});
		} else {
			this.setState({
	...
	_handlePasswordChange() {
		if (this.refs.signupPassword.value.length < 8){
			this.setState({
				passwordInvalid: "Password must be 8 or more characters.",
	...
	render() {
		return (
			...
			<Row>
			<Col sm="4" className="offset-sm-4" style={{marginTop: "100px"}}>
			<Card>
			<CardBlock>
			<CardTitle>Login</CardTitle>
			<div className={`form-group ${(this.state.emailInvalid ? "has-danger" : "")}`}>
			<label>Email Address</label>
			<input onBlur={this._handleEmailChange.bind(this)} onKeyDown={this._handleReturn.bind(this)} ref="signupEmail" type="text" className="form-control" />
			...
			<div className="form-group">
			<label><Link to="/forgot">Forgot your password?</Link></label>
			</div>
			<div className="form-group">
...
export class ForgotView extends Component {
	...
	render(){
		return (
			...
			<div className={`form-group ${(this.state.emailInvalid ? "has-danger" : "")}`}>
			<label>Email Address</label>
			<input onChange={this._handleEmailChange.bind(this)} onKeyDown={this._handleReturn.bind(this)}  ref="forgotEmail" type="text" className="form-control" />
			{this.state.emailInvalid ?
				<div className="form-control-feedback">{this.state.emailInvalid}</div>
				: <span></span>
			}
			...
			<label>Looking to login? <Link to="/login">Login</Link></label>
			</Col>
			</Row>
			</Container>
			);
...
export const Forgot = withRouter(connect({
	mapMutationsToProps() {
		return {
			forgot: (args) => ({
				mutation: gql`
...
export class RegisterView extends Component {
	...
	_handlePasswordChange(event) {
		if (this.refs.signupPassword.value.length < 8){
			...
			this.setState({
				passwordInvalid: "Passwords must match.",
				password: this.refs.signupPassword.value
			});
		} else {
			this.setState({
	...
	render() {
		return (
			...
			<div className={`form-group ${(this.state.passwordInvalid ? "has-danger" : "")}`}>
			<label>Repeat</label>
			<input onChange={this._handlePasswordChange.bind(this)} onKeyDown={this._handleReturn.bind(this)}  ref="signupPasswordRepeat" name="repeat" type="password" className="form-control" />
...
export const Register = withRouter(connect({
	mapMutationsToProps() {
		return {
			register: (args) => ({
				mutation: gql`
...
export class PasswordResetView extends Component {
	...
	render() {
		return (
			...
			<label>Remember your password? <Link to="/login">Login</Link></label>
			</Col>
			</Row>
			</Container>
			);
...
export const PasswordReset = withRouter(connect({
	mapMutationsToProps() {
		return {
			reset: (args) => ({
				mutation: gql`
